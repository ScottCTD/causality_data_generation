...
def facts_from_outcome(outcomes: Dict) -> List[OptionFact]:
    ...
    if hits == 0:
        ...
        if wall_hits and len(wall_hits) == hits and len(set(wall_hits)) == 1:
            ...
            # Otherwise, describe how many *distinct* walls were contacted.
            # Example: hits sequence [wall1, wall2, wall1] -> 2 different walls.
            distinct_walls = len(set(wall_hits)) if wall_hits else hits
            facts.append(OptionFact("hits_n_diff_walls", (distinct_walls,)))
...
def sample_multilabel_from_facts(
    ...
    The special ``\"not_pocketed\"`` fact is never surfaced as an answer option;
    if it is the only available fact, we indicate that no question should be
    generated by returning empty lists.
    """
    # Filter out the "not_pocketed" fact from candidates for correct options.
    filtered_true_facts: List[OptionFact] = [
        f for f in true_facts if f.kind != "not_pocketed"
    ]
    if not filtered_true_facts:
        # No permissible correct facts remain; signal caller to skip question.
        return [], []

    num_correct = max(1, min(num_correct, len(filtered_true_facts), total))

    # Choose which facts will be marked as correct.
    chosen_true: List[OptionFact] = random_sample(filtered_true_facts, num_correct)

    # Filter distractors so that:
    #   - they are not themselves true for this outcome, and
    #   - they are logically consistent with the chosen true facts, and
    #   - we avoid duplicates.
    chosen_set = set(chosen_true)
    true_set = set(filtered_true_facts)
    distractor_candidates: List[OptionFact] = []
    for f in pool_facts:
        if f.kind == "not_pocketed":
            continue
        # Do not use any fact that is actually true in this scenario, even if
        # we did not select it as a correct option, to avoid multiple true
        # answers appearing among distractors.
        if f in true_set or f in chosen_set:
            continue
        if not _is_consistent_distractor(f, chosen_true):
            continue
        distractor_candidates.append(f)
